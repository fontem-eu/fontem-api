...
from .load_exchange_rates import load_rates, to_eur

# Path to ECB exchange rates JSON (generated by load_exchange_rates.py)
_DEFAULT_RATES_PATH = os.environ.get(
    "ECB_RATES_PATH",
    os.path.join(os.path.dirname(__file__), "..", "..", "data", "ecb_rates.json"),
)
...
def load_contracts(driver, archive_path: Path, ecb_rates: dict | None = None):  # pylint: disable=too-many-locals
    ...
    with driver.session() as session:
        ...
        for notice in awards_and_modifications(
            ...
            for award in notice.awards:
                ...
                original_value = award.value or notice.total_value

                # Coalesce best available date
                effective_date, date_source = _coalesce_date(award, notice)

                # Convert to EUR using ECB daily rate (or None if unknown)
                value_eur = None
                if ecb_rates and original_value is not None:
                    rate_date = effective_date or notice.issue_date
                    value_eur = to_eur(original_value, currency or "EUR", rate_date, ecb_rates)
...
def main(argv=None):
    ...
    parser.add_argument(
        "--rates-file",
        default=os.environ.get("ECB_RATES_PATH", _DEFAULT_RATES_PATH),
        help="Path to ECB exchange rates JSON (from load_exchange_rates.py)",
    )
    ...
    if rates_path.exists():
        ecb_rates = load_rates(str(rates_path))
        logger.info("Loaded ECB rates for %d currencies", len(ecb_rates))
    else:
        logger.warning("No ECB rates file at %s — EUR conversion will be skipped", rates_path)
    ...
    try:
        ...
        load_contracts(driver, archive, ecb_rates=ecb_rates)
    finally:
        driver.close()
