...
Uses edgartools' EntityFacts API — no network calls for fundamental data.
The ticker-list endpoint reads from the local reference/company_tickers.json.

Concept matching uses XBRL concept name lists defined below, filtering to only
names without spaces (i.e. actual XBRL concept names, not human-readable
labels).  Concepts are looked up as "us-gaap:<ConceptName>".
"""
...
# ---------------------------------------------------------------------------
# XBRL concept name priority lists (most standard / most common first)
# Only entries without spaces are used as XBRL concept identifiers.
# ---------------------------------------------------------------------------
...
_TOTAL_ASSETS = ["Total assets", "Total Assets", "Assets"]
_TOTAL_LIABILITIES = [
...
_CURRENT_ASSETS = ["Total current assets", "Current assets", "AssetsCurrent"]
...
_PREPAID_EXPENSES = [
    ...
    "PrepaidExpenseAndOtherAssets", "PrepaidExpenseCurrent",
    "Prepaid expense and other assets", "Other current assets",
]
_CAPEX = [
    ...
    "Capital expenditures, net", "CapitalExpenses",
]
_GROSS_PROFIT = [
...
_INTEREST_EXPENSE = [
    ...
    "Interest expense, net", "Interest and debt expense",
    "InterestAndDebtExpense", "Finance costs",
